        ...
        if ingest_data_button:
            if session.data_type == "URL":
                ...
                with st.spinner("Extracting..."):
                    start_time = time.perf_counter()
                    if session.file_extension == "csv":
                        docs = loadCSV(f"./uploaded_files/{session.file_name}")
                    elif session.file_extension == "xlsx":
                        docs = loadExcel(f"./uploaded_files/{session.file_name}")
                    else:
                        docs = []
